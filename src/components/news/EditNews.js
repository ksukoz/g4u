	...
	componentWillReceiveProps(nextProps) {
		if (nextProps.errors || nextProps.messages) {
			...
			this.setState({
				...
				tags: nextProps.news.newsItem.tags.length > 0 ? nextProps.news.newsItem.tags.length : []
			});
		}
	}
	...
	render() {
		...
		if (tags.length > 0) {
			tagList = tags.map((tag, i) => {
				return <Chip key={i} label={tag} onDelete={this.onDeleteHandle(i)} className={classes.chip} />;
			});
		}
								...
								<CKEditor
									...
									config={{
										toolbar: [
											...
											[ 'Image', 'Table', '-', 'Flash', 'Smiley', 'TextColor', 'BGColor' ]
										]
									}}
								/>
							</div>
							<InputFile type="image" name={this.state.photo} onChange={this.onChangeFileHandler} />
							<div>
								<TextField
									type="text"
									name="tag"
