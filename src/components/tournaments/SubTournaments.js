	...
	render() {
				...
				<ExpansionPanel key={season.season_id}>
					...
					{season.tourney ? (
						season.tourney.map((tourneyItem) => (
							<ExpansionPanelDetails key={tourneyItem.tourneyId} className={classes.expDetails}>
								<Link to={`/subtourinfo/${tourneyItem.tourneyId}`}>{tourneyItem.title}</Link>
		...
		return (
			<div className={classes.container}>
				...
				{subTournaments ? <img src={subTournaments.logo} alt="" style={{ width: '100%' }} /> : ''}
...
export default compose(withStyles(styles), connect(mapStateToProps, { getSubTournaments }))(SubTournaments);
